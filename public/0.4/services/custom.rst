...
We can now push this service to dotCloud::
  ...
  dotcloud push mycustomservice my-custom-service
...
``dotcloud var set`` allows to expose variables without having to
...
be overridden by ``dotcloud var set`` if necessary.
