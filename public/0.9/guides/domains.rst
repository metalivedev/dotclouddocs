...
New Custom Domains require the Live or Enterprise flavor. So the first steps in
...
* EveryDNS.net
* Yahoo!SmallBusiness
* GoDaddy.com (note: they do not support wildcard domains in CNAMEs_)
...
the value returned by ``dotcloud domain`` after "IN CNAME".
...
registrar. In general you follow these steps:
   ...
   d. Add a CNAME record
      ...
      ii. *Host Name* or *Data* is the value ``dotcloud domain add`` returned after "IN CNAME", e.g. "gateway.dotcloud.com".

dotCloud's domain command will be effective in about a minute after
...
You cannot use a "naked domain" (i.e. an address without a host part, like
my-own-domain.com).

.. note::

   You can use *wildcard domains*, by using a domain like \*.example.com.
   Not all providers allow wildcards, however. Please check before you buy.
...
.. _SSL: ../ssl
.. _CNAMEs: http://support.godaddy.com/groups/domains-management-and-services/forum/topic/wildcard-cname/
