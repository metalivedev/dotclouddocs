...
you can fix it manually with ``dotcloud run``, or use tar options to
...
#. Append the following section to your ``~/.ssh/config`` file (create this
   file if it does not exist), replacing USER, HOST and PORT with the values
   retrieved from  ``dotcloud info``::
