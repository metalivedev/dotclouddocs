...
:keywords: dotCloud documentation, dotCloud service, PHP service, PHP5, Drupal, Joomla, WordPress, PHPBB
...
The *PHP* service can host any kind of PHP5 based website: 
- Frameworks like Symfony or CakePHP, 
- Content Management Systems like Drupal or Joomla, WordPress or PHPBB,
- and of course custom-tailored applications. 
...
- if you are an Apache expert, you might be confused at first by Nginx
  ...
  directives will often be expressed in a simpler way with Nginx syntax.
...
need to know where it lives, on which port, you will also certainly need to know the right 
...
application's top-level directory. This file is included after all other PHP
directives, so you can override anything you need.

For example, if you want to display all errors (very convenient during
development to make sure that you spot even the slightest warnings and
notices):
...
Nginx is covered in detail in the :doc:`Nginx guide </guides/nginx>`.
...
.. note::
   ...
   That means that you can override most of our default Nginx configuration including
   the location block that handles PHP requests.
